...
export function ActionConfirmModal({
  ...
  const remarkRequired = isProcess;
    ...
    >
      <div style={{ marginBottom: 16 }}>
        <Text>
          ...
          <Text strong type={isProcess ? "success" : "danger"}>{` ${label} `}</Text>
          操作，是否继续？
        </Text>
      </div>
      <div>
        <Text style={{ display: "block", marginBottom: 8 }}>
          {isProcess ? "封号原因 / 备注（必填）" : "撤销备注（选填）"}
        </Text>
        <Input.TextArea
          rows={3}
          maxLength={200}
          showCount
          placeholder={isProcess ? "请填写封号原因，例如：多端同时在线证据充分" : "选填，可说明撤销理由"}
          value={remark}
